...
드디어 YOLOv8n을 INT8로 양자화해서 Orange Pi 5의 NPU에 올려봤다.\\
...
로컬에서 RKNN 모델 변환하는 게 귀찮아서 `build_rknn.yml` 워크플로우를 만들어뒀는데, 돌릴 때마다 새로운 에러가 튀어나왔다.\\
...
**1차: 파라미터명이 달랐다.** rknn-toolkit2가 v2.x로 올라오면서 `quantization_algorithm`이 `quantized_algorithm`으로 이름이 바뀌어 있었다. 문서랑 실제 API가 따로 놀고 있었던 것이다.\\
...
**2차: onnx 버전이 문제였다.** `onnx>=1.16.1`을 그대로 뒀더니 빌드가 깨졌다. `onnx.mapping` 모듈이 1.17부터 아예 없어졌기 때문이었다. `onnx>=1.14.0,<=1.16.0`으로 못 박아서 해결했다.\\
...
**3차: urllib이 리다이렉트를 못 따라갔다.** INT8 캘리브레이션용 COCO128 데이터셋을 받는 스텝에서 urllib이 308 리다이렉트를 처리하지 못해 다운로드가 실패했다. `wget`으로 바꾸는 걸로 끝냈다.\\
...
세 번의 사고를 다 넘기고 나서야 `yolov8n.rknn`(4.8MB)이 빌드됐고, `scp`로 보드에 옮겼다.\\
...
모델은 빌드됐는데, `vision/rknn_detector.py`에서 돌려보니 이번엔 다른 버그 두 개가 기다리고 있었다.\\
The model was built, but running it through `vision/rknn_detector.py` turned up two different bugs.

하나는 입력 텐서 차원이었다. `(H,W,C)` 형태로 그냥 넘기고 있었는데, `np.expand_dims`로 배치 차원을 명시해서 `(1,H,W,C)`로 만들어줘야 했다.\\
...
다른 하나는 NPU 코어 설정 누락이었다. `init_runtime(core_mask=RKNNLite.NPU_CORE_0_1)`을 넣어주지 않으면 듀얼 코어 중 하나만 쓰고 있었다.\\
