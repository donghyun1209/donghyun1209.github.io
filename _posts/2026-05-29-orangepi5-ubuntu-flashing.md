...
오렌지파이 5(4GB RAM) 보드를 구동하기 위해 맥북 에어 환경에서 128GB 고성능 마이크로 SD카드에 **Ubuntu 22.04 Jammy Server** OS를 빌드하면서 겪은 트러블슈팅 기록입니다.
This is a troubleshooting record I went through building the **Ubuntu 22.04 Jimmy Server** OS on a 128GB high-performance micro SD card in a MacBook Air environment to run an Orange Pie 5 (4GB RAM) board.

결론부터 말하자면, 보드나 SD카드가 아니라 **'저가형 SD카드 리더기'**가 원인이었습니다.
...
가장 대중적인 플래싱 툴인 `balenaEtcher`를 이용해 압축을 푼 `.img` 파일을 구우려고 시도했습니다. 그러나 게이지가 채 시작되기도 전에 에러가 발생했습니다.
...
macOS의 깐깐한 보안 정책 때문에 Etcher가 디스크 깊숙한 곳에 접근하는 것이 차단된 것으로 판단하여, 시스템 설정에서 **[전체 디스크 접근 권한]**을 부여하고 에디터를 재실행했으나 동일한 증상이 반복되었습니다.
...
Etcher의 버그를 의심하여 더 안정적인 `Raspberry Pi Imager`로 툴을 전환했습니다. 이번에는 게이지가 올라가는 듯했으나 중간에 멈춰버렸습니다.
...
디스크 파티션이 꼬여 강제 포맷을 시도했으나, 이마저도 `Waiting for partitions to activate` 단계에서 무한 프리징에 걸렸습니다. 결국 디스크 유틸리티 앱을 강제 종료하고 맥북을 재부팅해야 했습니다.\\
...
두 가지 플래싱 툴과 OS 포맷 프로세스가 모두 특정 구간에서 뻗어버리는 현상을 분석한 결과, **물리적 연결 칩셋의 과열**이 원인임을 알아냈습니다.\\
...
현재 사용 중인 임시방편용 **다이소 가성비 리더기**가 문제였습니다.\\
