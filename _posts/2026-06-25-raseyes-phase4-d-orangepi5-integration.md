...
원인은 타이밍 버짓이었다. 센서를 LONG 모드(최대 거리 측정용, 최소 140ms 타이밍 버짓 필요)로 설정해뒀는데, 실제 폴링 주기는 50ms로 짜여 있었다. 센서가 요구하는 시간보다 훨씬 짧은 주기로 계속 값을 요청하니 매번 "아직 준비 안 됐다"는 경고만 쌓이는 것이었다.\\
...
오늘 배운 건, 경고 로그가 쌓인다고 무조건 배선이나 전원을 의심할 게 아니라 설정값과 실제 주기가 서로 맞는지부터 확인해야 한다는 것이었다. 스펙 문서를 안 읽고 감으로 값을 넣었다가 한참 헤맸다.\\
