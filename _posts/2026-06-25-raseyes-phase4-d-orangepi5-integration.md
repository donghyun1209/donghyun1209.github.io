...
의존성 설치를 마치고 드디어 보드에서 통합 테스트를 돌려봤습니다.\\
...
`sounddevice`, `gpiod`, `VL53L1X` 라이브러리부터 깔았습니다. 매번 손으로 카메라 켜보고 센서 값 찍어보는 게 귀찮아서, `scripts/test_device.py`를 만들어 CSICameraHAL과 VL53L1XHAL이 제대로 동작하는지 한 번에 확인할 수 있게 했습니다. 나중에 NPU 성능도 재야 할 것 같아서 `scripts/bench_rknn.py`도 미리 만들어뒀습니다.\\
...
`vl53l1x_hal.py`에서 파라미터명 오타(`i2c_port` → `i2c_bus`)부터 잡았습니다. 여기까진 쉬웠습니다.\\
First I fixed a parameter name typo in `vl53l1x_hal.py` (`i2c_port` → `i2c_bus`). That part was easy.

진짜 문제는 그다음이었습니다. 거리값이 갱신되는 속도가 점점 느려지더니 거의 1초에 한 번꼴로만 값이 바뀌었고, 로그에는 만료 경고가 쉴 새 없이 쌓였습니다. 처음엔 I2C 배선이나 전원 문제인 줄 알았습니다.\\
...
원인은 타이밍 버짓이었습니다. 센서를 LONG 모드(최대 거리 측정용, 최소 140ms 타이밍 버짓 필요)로 설정해뒀는데, 실제 폴링 주기는 50ms로 짜여 있었습니다. 센서가 요구하는 시간보다 훨씬 짧은 주기로 계속 값을 요청하니 매번 "아직 준비 안 됐다"는 경고만 쌓이는 것이었습니다.\\
...
모드를 MEDIUM(최대 3m, 33ms+ 버짓)으로 낮췄습니다. 50ms 주기에서도 여유가 생겨서 안정적으로 약 10Hz 샘플링이 나왔고, 경고도 싹 사라졌습니다. 어차피 RasEyes가 감지해야 하는 장애물 거리는 3m면 충분했습니다.\\
...
`rknnlite2`나 `ultralytics`, `PortAudio`가 설치 안 된 환경에서 `main.py`가 그냥 죽어버리는 것도 손봤습니다. 특히 `_build_vision()`에서는 `.rknn` 모델 파일이 있는지부터 미리 검사해서, 없으면 `MockVision`으로 조용히 넘어가게 만들었습니다.\\
...
`systemd` 등록도 마쳤습니다. 부팅 시간을 재보니 커널 3.8초, 유저스페이스 7.2초로 총 11초. 목표였던 45초에 비하면 훨씬 여유 있는 수치라 만족스러웠습니다.\\
...
`sounddevice`가 시스템에 깔린 `PortAudio`를 알아서 잘 찾아줘서, `JackAudioHAL`은 별다른 수동 설치 없이 바로 돌아갔습니다.\\
...
PyTorch 모델을 RKNN 파일로 변환하는 작업을 매번 로컬에서 하기 귀찮아서, GitHub Actions에 `build_rknn.yml` 워크플로우를 만들어뒀습니다.\\
...
지금 Orange Pi 5는 `MockVision` + `VL53L1XHAL` + `JackAudioHAL` 조합으로 거리 기반 경보는 정상 동작 중입니다. CSI 카메라는 `/dev/video11` 드라이버 한계로 목표였던 15 FPS에 못 미치는 10 FPS 정도가 나오는데, `yolov8n.rknn`만 배포하면 바로 NPU 추론으로 갈아탈 준비는 끝나 있습니다.\\
...
오늘 배운 건, 경고 로그가 쌓인다고 무조건 배선이나 전원을 의심할 게 아니라 설정값과 실제 주기가 서로 맞는지부터 확인해야 한다는 것이었습니다. 스펙 문서를 안 읽고 감으로 값을 넣었다가 한참 헤맸습니다.\\
